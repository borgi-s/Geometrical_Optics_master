...
- [ ] **Delete the globals + import-time geometry derivations:** `theta_0`, `theta`, `Theta`, `xl_start`, `xl_range`, `rl`, `prob_z` (and their import-time construction lines), `Resq_i`, `qi1_range`/`qi2_range`/`qi3_range`, `qi1_start/step`…`qi3_start/step`, `npoints1/2/3`, `qi_starts`, `qi_steps`, `_analytic_eval`, `Hg`, `q_hkl`, `_loaded_kernel_path`. **KEEP** instrument constants (`psize`, `zl_rms`, `Npixels`, `Nsub`, `NN1/2/3`, `Us`, `Ud`, `_flat_indices`, `yl_start`, `xl/yl/zl_steps`, **`yl_range`, `zl_range`**). **NOTE (corrected after S3):** `xl_start` and `xl_range` are θ-DEPENDENT geometry (`xl_start = yl_start/tan(2θ)/3`; they are `GeometryContext` fields + in the CM's mutation list) → they are DELETED, NOT kept. Only `yl_range` (= `-yl_start`) and `zl_range` (= `-zl_start = 3·zl_rms`) are θ-independent constants that stay.
- [ ] **`Z_shift` must stop reading the `xl_range` global.** S3 added an `xl_range_override` kwarg (default `None` → reads the global) and all 4 call sites pass `ctx.geometry.xl_range`. Once the `xl_range` global is deleted here, the `None` default would `NameError` — so make `xl_range_override` REQUIRED (rename to `xl_range`, positional/keyword-required) or have `Z_shift` take the value unconditionally. Update the 4 call sites + any test caller accordingly.
