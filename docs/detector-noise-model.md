...
> **Update (2026-06-18) — provisional default lowered to `1.0e4 / 15` (≈ 667 ADU/s).**
> The §5 record below (the 2026-06-13 integral-anchored 2.90×10⁶ derivation and the
> "stays at the provisional `1.0e4`" decision) is a **superseded historical record**,
> kept verbatim. The integral anchor was later traced to a wrong-feature artifact, and
> the rocking-curve study
> ([`docs/calibration/counts_scale_rocking_study_2026-06-16.md`](calibration/counts_scale_rocking_study_2026-06-16.md))
> puts the true value at O(10²–10³) (weak-beam regime ~400–800), making `1.0e4`
> ~15–30× too high. As a quick provisional fix `DetectorConfig.counts_scale` now
> defaults to `1.0e4 / 15 ≈ 667`, which lands in that range. **The pin is still OPEN**
> (a centred-pristine rocking scan would pin ~500–700). Brightness remains user-tunable
> via `exposure_time` (`adu = ideal × counts_scale × exposure_time`).
