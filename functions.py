...
def Fd_find_edge(rl, Ud, Us, Theta, dis = 1, ndis = 1, b = 2.862e-4, ny = 0.334, misorientation = False, t_vec = None):
    '''
    Calculates the displacement field due to multiple edge dislocations in a 
    crystal lattice, given the lab coordinates and accompanying rotation matrices.
    The dislocation wall will start at the center of the system and branch out as 
...
def Fd_find_screw(rl, Ud, Us, Theta, dis = 1, xl_steps = 170, b = 2.862e-4, ny = 0.334, misorientation = False, t_vec = None):
    '''
    Calculates the displacement field due to multiple edge dislocations in a 
    crystal lattice, given the lab coordinates and accompanying rotation matrices.
    The dislocation wall will start at the center of the system and branch out as 
    ...
    #         # For even numbers go in negative xd direction
    #         if i%2 == 0:
    #             rd_new[0] -= 2.394e-08 * count1 # shift med pixel afstand i xd
    #             count1 += 1
            
    #         # For uneven numbers go in positive xd direction
    #         if i%2 == 1:
    #             rd_new[0] += 2.394e-08 * count2 # shift med pixel afstand i xd
    #             count2 += 1
            
    #         sqy = rd_new[1] * rd_new[1] # yd
    #         sqz = rd_new[2] * rd_new[2] # zd
    #         denom = (sqz) + (sqy) + alpha

    #         # Add the components of the deformation gradient tensor 
    #         # for each new dislocation.
    #         Fdd[:, 0, 1] + -rd_new[2] / denom
    #         Fdd[:, 0, 2] + rd_new[1] / denom
...
def rotate_matrix_z_axis(matrix, angle_degrees):
    ...
    # Rotate the matrix by multiplying it with the Z-axis rotation matrix
    rotated_matrix = np.dot(Rz, matrix)
