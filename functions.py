...
def rotatedU(axis, alpha, U, coordtype):
    """
    Rotate matrix U by angle alpha around the given axis.

    Parameters:
    axis (array-like): Rotation axis vector.
    alpha (float): Rotation angle in degrees.
    U (array-like): Rotation matrix to be rotated.
    coordtype (str): Coordinate system type ('sample' or 'cryst').

    Returns:
    ndarray: Rotated rotation matrix.
    """
...
def check_folder(path, folder_name):
    ...
    Parameters:
    path (str): The directory path where the folder should be checked/created.
    folder_name (str): The name of the folder to check/create.

    Prints:
    Message indicating whether the folder was created or already exists.
    """
...
def image_range(im_in, lower, upper):
    """
    Rescale the pixel values of an image to a specified range.

    Parameters:
    im_in (ndarray): Input image array.
    lower (float): The lower bound of the output range.
    upper (float): The upper bound of the output range.

    Returns:
    ndarray: Image array with values rescaled to the range [lower, upper].
    """
...
def Fd_find(rl, Ud, Us, S, Theta, dis = 1, ndis = 1, b = 2.862e-4, ny = 0.334, misorientation = False, t_vec = None):
    '''
    ...
    if ndis > 100:
        ...
        njobs = cpu_count() - 4 # number of processes
...
def load_or_generate_Hg(rl, Ud, Us, S, Theta, dis, ndis, file_path=None):
    """
    Load or generate the Hg matrix.

    Parameters:
    rl (ndarray): Lab coordinates.
    Ud (ndarray): Rotation matrix to/from dislocations coordinates.
    Us (ndarray): Rotation matrix to/from sample coordinates.
    S (ndarray): Rotation matrix for different reflections.
    Theta (ndarray): Rotation matrix to/from imaging coordinates.
    dis (float): Distance between dislocations.
    ndis (int): Number of dislocations.
    file_path (str, optional): Path to save/load the Fg array.

    Returns:
    ndarray: The Hg matrix.
    """
    if file_path is not None:
        ...
        try:
            ...
            if ndis == 0:
                ...
                Fg = Fd_find(rl * 1e6, Ud, Us, S, Theta, dis, ndis)
...
def square(list):
    """
    Return a list of the squares of each number in the input list.

    Parameters:
    list (list of int/float): List of numbers to be squared.

    Returns:
    list of int/float: List with squared values.
    """
...
def fast_inverse2(A): 
    """
    Compute the inverse of a batch of 3x3 matrices.

    Parameters:
    A (ndarray): Batch of 3x3 matrices with shape (N, 3, 3).

    Returns:
    ndarray: Batch of inverses of the input matrices with shape (N, 3, 3).
    """
    inv = np.empty_like(A)
    a = A[:,0, 0]
    b = A[:,0, 1]
