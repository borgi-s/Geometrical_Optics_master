...
from functions import (fast_inverse2, load_or_generate_Hg, 
                       rotatedU, Fd_find_edge, Fd_find_screw, Fd_find_mixed)
...
theta_0 = 20.76/2*np.pi/180 # in rad
...
# pkl_fn = 'Resq_i_20230913_1308.pkl' # Change accordingly 111 reflection
pkl_fn = 'Resq_i_20240913_1359.pkl' # Change accordingly 020 reflection
...
theta = theta_0
yl_start = (-psize * NN2 / 2 + psize / (2 * Nsub))  # start in yl direction, in unit of m, centered at 0
xl_start = (-psize * NN1 / 2 + psize / (2 * Nsub)) / np.tan(2 * theta) # start in xl direction, in m, for zl=0
...
# # Us = np.array([[1 / np.sqrt(2), -1 / np.sqrt(6), q_hkl[0]],
# #                [0, -2 / np.sqrt(6), q_hkl[1]],
# #                [-1 / np.sqrt(2), -1 / np.sqrt(6), q_hkl[2]]]).T
# Theta = np.array([[np.cos(theta), 0, np.sin(theta)],
#                   [0, 1, 0],
#                   [-np.sin(theta), 0, np.cos(theta)]])


# # # SCREW: Create matrices according to Eqs 2,3,7
...
# Ud_screw = np.array([np.cross(n,t), n, t]).T
# Ud_edge = np.asarray([b, n, t]).T
# b_vec = np.array([1,-1,0])/np.sqrt(2)
# n = np.array([1, 1, -1])/np.sqrt(3)
# t_screw = np.array([1, -1, 0])/np.sqrt(2)
# t_edge = np.array([1, 1, 2])/np.sqrt(6)

# alpha_angle = 0

# Ud_screw = np.array([np.cross(n,t_screw), n, t_screw]).T
# Ud_edge = np.array([b_vec, n, t_edge]).T
'''

# def find_Ud_mix(n,t_edge,t_screw,alpha_angle):
#     t_mixed = (np.cos(np.deg2rad(alpha_angle))*t_edge +
#             np.sin(np.deg2rad(alpha_angle))*t_screw)
#     return np.asarray([np.cross(n,t_mixed), n, t_mixed]).T
...
def Find_Hg(dis, ndis, psize, Ud, zl_rms, screw = False, edge = False):


    Fg_path = sys.path[0]+'/direct_space/deformation_gradient_tensors/Fg_comparison3_{0}_{1}nm_{2}nm.npy'.format(str(dis).replace('.', ''), int(psize*1e9), int(zl_rms*2.35e9))
    if screw == True and edge == False:
        Fg_path = sys.path[0]+'/direct_space/deformation_gradient_tensors/Fg_screw_{0}_{1}nm_{2}nm.npy'.format(str(dis).replace('.', ''), int(psize*1e9), int(zl_rms*2.35e9))
    if screw == False and edge == True:
        Fg_path = sys.path[0]+'/direct_space/deformation_gradient_tensors/Fg_edge_{0}_{1}nm_{2}nm.npy'.format(str(dis).replace('.', ''), int(psize*1e9), int(zl_rms*2.35e9))
    if screw == True and edge == True:
        Fg_path = sys.path[0]+'/direct_space/deformation_gradient_tensors/Fg_mixed_{0}_{1}nm_{2}nm.npy'.format(str(dis).replace('.', ''), int(psize*1e9), int(zl_rms*2.35e9))


    Hg = load_or_generate_Hg(rl, Ud, Us, Theta, dis, ndis, Fg_path, screw, edge)
...
# Fg_edge = Fd_find_edge(rl*1e6, Ud_edge, Us, Theta)
# Fg_screw = Fd_find_screw(rl*1e6, Ud_screw, Us, Theta)
# Fg = np.cos(np.deg2rad(45))*Fg_edge + np.sin(np.deg2rad(45))*Fg_screw
# Fg += np.identity(3)

# Fg1 = Ud_screw @ Fg @ Ud_screw.T

# Fg = Fd_find_mixed(rl*1e6, Us, Ud_mix, alpha_angle, Theta)
...
# Us = np.array([[1 / np.sqrt(6),    -2 / np.sqrt(6),   1 / np.sqrt(6)],
#                [ 1 / np.sqrt(2),    0,                -1 / np.sqrt(2)],
#                [ 1 / np.sqrt(3),   1 / np.sqrt(3),   1 / np.sqrt(3)]], dtype = np.float64)
...
Us = np.array([[1 / np.sqrt(2),    0,   -1 / np.sqrt(2)],
               [ -1 / np.sqrt(2),    0,                -1 / np.sqrt(2)],
               [ 0,   2 / np.sqrt(4), 0]], dtype = np.float64)
...
def forward(Hg, phi = 0, chi = 0, TwoDeltaTheta = 0, prob_z = None, qi_return = False):
    ...
    qs = np.einsum('ij, kjp, p -> ki', Us, Hg, q_hkl)
    ...
    index1 = np.int16( (qi[0] - qi1_start)/qi1_step )
    index2 = np.int16( (qi[1] - qi2_start)/qi2_step )
    index3 = np.int16( (qi[2] - qi3_start)/qi3_step )
    ...
    # Create a sparse matrix from indices and pro and add to im_1
    im_1 = coo_matrix((pro, (indices[:, 0], indices[:, 1])), shape=((NN2//Nsub), NN1//Nsub)).toarray()
