...
psize = 50E-9 # pixel size in units of m, in the object plane
...
# INPUT FOV
Npixels = 410 # nr of pixels on detector (same in both y and z) - sets the FOV.
...
pkl_fn = 'Resq_i_20231107_0952.pkl' # Change accordingly
...
S4 = np.array([[1/3,  2/3,  2/3], # S4
             [2/3,  1/3, -2/3],
             [-2/3, 2/3, -1/3]])
S3 = np.array([[1/3,  -2/3, 2/3], # S3
             [2/3,   2/3,  1/3],
             [-2/3,  1/3,  2/3]])
S2 = np.array([[1/3,  -2/3,-2/3], # S2
             [2/3,  -1/3, 2/3],
             [-2/3, -2/3, 1/3]])
S1 = np.identity(3)
...
ndis = 121 # number of dislocations
dis = 4 # units of micrometer
def Find_Hg(xl_start, yl_start, zl_start, dis, ndis, psize, zl_rms, I = np.identity(3), h = -1, k = 1, l = -1, offset=0):
    ...
    Fg_path = sys.path[0]+'/direct_space/Testing/Fg_rg_{0}_{1}nm_{2}nm_zl_offset_{3}nm.npy'.format(str(dis).replace('.', ''), int(psize*1e9), int(zl_rms*2.35e9), int(offset*1e9))
    Hg = load_or_generate_Hg(rl, Ud, Us, S, Theta, dis, ndis, Fg_path)
    ...
    return Hg, q_hkl, np.exp(-0.5*((rl[2]+offset)/zl_rms)**2)




S = S1
Hg, q_hkl, prob_z = Find_Hg(xl_start, yl_start, zl_start, dis, ndis, psize, zl_rms)
# np.save('Test_S1_Hg_rg.npy',Hg)
...
def forward(Hg, phi = 0, chi = 0, TwoDeltaTheta = 0, qi_return = False):
    ...
    numpy.ndarray, numpy.ndarray
        The scattering vector, qi_field, in imaging space, shape(3,X), X being (NN1 * NN2 * NN3)
        The forward modelled image, im_1, shape (NN3,NN1)
