...
extraction in the original ``init_forward.py``. It consumes image stacks
...
.. note:: ``compute_chi_shift`` removed (post-2.2.0)

    Earlier versions exported a ``compute_chi_shift`` helper that measured a
    scalar χ-axis offset from the corner pixel of a strain-free reference
    stack. It was retired: it carried an off-by-half-cell baseline bias
    (~100× the mosaicity signal) and an ``abs()`` sign-loss, and the live
    ``run_postprocess`` already read the χ axis off the nominal grid with
    ``chi_shift=0.0`` rather than calling it. The ``chi_shift`` keyword on
    :func:`compute_com_maps` remains for callers that want to apply an
    externally-determined additive offset.
"""
...
def compute_com_maps(
    ...
    *,
    ...
    Args:
        ...
        chi_shift: Additive shift to the χ axis (radians). Defaults to 0.0;
            supply an externally-determined offset if the χ grid needs
            calibrating.
