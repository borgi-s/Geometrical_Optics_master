...
def cli_main(argv: list[str] | None = None) -> int:
    ...
    if multi:
        ...
        for run in runs:
            ...
            # Honour --if-missing / --force with the same semantics as single mode.
            if kernel_path.exists():
                if args.if_missing:
                    print(f"kernel already present at {kernel_path}; skipping.")
                ...
                if not args.force:
                    print(
                        f"refusing to overwrite existing kernel npz at {kernel_path}; "
